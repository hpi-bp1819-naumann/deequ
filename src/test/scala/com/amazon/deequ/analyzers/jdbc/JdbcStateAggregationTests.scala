...
class JdbcStateAggregationTests extends WordSpec with Matchers with JdbcContextSpec
  ...
  "State aggregation outside" should {
    ...
    "give correct results" in {
      ...
      correctlyAggregatesStates(Size())
      correctlyAggregatesStates(Maximum("marketplace_id"))
      correctlyAggregatesStates(Minimum("marketplace_id"))
      correctlyAggregatesStates(Mean("marketplace_id"))
      //correctlyAggregatesStates(StandardDeviation("marketplace_id"))
      correctlyAggregatesStates(Uniqueness("attribute" :: "value" :: Nil))
      correctlyAggregatesStates(Distinctness("attribute" :: Nil))
      correctlyAggregatesStates(CountDistinct("value" :: Nil))
      correctlyAggregatesStates(UniqueValueRatio("value"))
      correctlyAggregatesStates(Completeness("attribute"))
      correctlyAggregatesStates(Compliance("attribute", "attribute like '%facets%'"))
      //correctlyAggregatesStates(Correlation("numbersA", "numbersB"))
      correctlyAggregatesStates(Entropy("attribute"))
      //correctlyAggregatesStates(Histogram("value"))
    }
  }

  def correctlyAggregatesStates[S <: State[S]](analyzer: Analyzer[S, Metric[_]])
  : Unit = {
