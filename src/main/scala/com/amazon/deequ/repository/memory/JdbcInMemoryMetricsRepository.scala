...
class JdbcInMemoryMetricsRepository() extends JdbcMetricsRepository {

  private val resultsRepository = new ConcurrentHashMap[ResultKey, JdbcAnalysisResult]()
  ...
  def save(resultKey: ResultKey, analyzerContext: JdbcAnalyzerContext): Unit = {
    ...
    resultsRepository.put(resultKey,
      deequ.repository.JdbcAnalysisResult(resultKey, analyzerContextWithSuccessfulValues))
  }
