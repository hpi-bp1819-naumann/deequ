...
import com.amazon.deequ.runtime.jdbc.operators.Operator
...
case class OperatorResults(metricMap: Map[Operator[_, Metric[_]], Metric[_]]) {
  ...
  def metric(operator: Operator[_, Metric[_]]): Option[Metric[_]] = {
    metricMap.get(operator)
  }
}
...
object OperatorResults {
  ...
  def successMetricsAsJson(operatorContext: OperatorResults,
                           forAnalyzers: Seq[Operator[_, Metric[_]]] = Seq.empty): String = {

    val metricsList = getSimplifiedMetricOutputForSelectedAnalyzers(operatorContext, forAnalyzers)
  ...
  private[this] def getSimplifiedMetricOutputForSelectedAnalyzers(
                                                                   operatorContext: OperatorResults,
                                                                   forAnalyzers: Seq[Operator[_, Metric[_]]])
  : Seq[SimpleMetricOutput] = {

    val selectedMetrics = operatorContext.metricMap
