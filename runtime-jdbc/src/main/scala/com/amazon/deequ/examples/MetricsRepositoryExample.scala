...
object MetricsRepositoryExample extends App {
  ...
  */
  withJdbcForSQLite { connection =>
    ...
    val metricsFile = new File(Files.createTempDirectory(UUID.randomUUID().toString).toFile, "metrics.json")
    ...
    val repository: MetricsRepository = DiskMetricsRepository(metricsFile.getAbsolutePath)
