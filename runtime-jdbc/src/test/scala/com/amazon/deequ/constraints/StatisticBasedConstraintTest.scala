...
class StatisticBasedConstraintTest extends WordSpec with Matchers with JdbcContextSpec
  ...
  "Analysis based constraint" should {
    ...
    "get the analysis from the context, if provided" in withJdbc { connection =>
      val df = getTableMissing(connection)
    ...
    "fail on analysis if value picker is provided but fails" in withJdbc { connection =>
    ...
    "return failed constraint for a failing assertion" in withJdbc { session =>
      val msg = "-test-"
      val exception = new RuntimeException(msg)
      val df = getTableMissing(session)
