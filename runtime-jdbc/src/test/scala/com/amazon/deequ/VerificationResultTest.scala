...
class VerificationResultTest extends WordSpec with Matchers with JdbcContextSpec
  with FixtureSupport {

  "VerificationResult getSuccessMetrics" should {

//    "correctly return a DataFrame that is formatted as expected" in withJdbc { connection =>
//
//      evaluate(connection) { results =>
//
//        val successMetricsAsDataFrame = VerificationResult
//          .successMetricsAsDataFrame(connection, results)
//
...
//
//          val successMetricsAsDataFrame = VerificationResult
//            .successMetricsAsDataFrame(connection, results, metricsForAnalyzers)
//
//          import connection.implicits._
//          val expected = Seq(
//            ("Column", "att1", "Completeness", 1.0),
...
//
//        val successMetricsAsDataFrame = VerificationResult
//          .checkResultsAsDataFrame(connection, results)
//
//        import connection.implicits._
//        val expected = Seq(
...
//    }
  ...
  private[this] def evaluate(connection: Connection)(test: VerificationResult => Unit): Unit = {
