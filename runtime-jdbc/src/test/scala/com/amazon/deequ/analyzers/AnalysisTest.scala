...
class AnalysisTest extends WordSpec with Matchers with JdbcContextSpec with FixtureSupport {
  ...
  "Operators" should {
    ...
    "return basic statistics" in withJdbc { connection =>
      ...
      val resultMetrics = analysis.run(table).allMetrics

      assert(resultMetrics.size == analysis.operators.size)
    ...
    "return a custom generic exception that wraps any other exception that " +
      "occurred while calculating the metric" in withJdbc { connection =>
        val df = getTableWithNumericValues(connection)
