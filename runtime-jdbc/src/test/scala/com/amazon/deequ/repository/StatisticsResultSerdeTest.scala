...
class StatisticsResultSerdeTest extends FlatSpec with Matchers {

  "analysis results serialization with successful Values" should "work" in {

    val analyzerContextWithAllSuccValues = ComputedStatistics(Map(
    ...
    val analysisResultOne = StatisticsResult(resultKeyOne, analyzerContextWithAllSuccValues)

    val analysisResultTwo = StatisticsResult(resultKeyTwo, analyzerContextWithAllSuccValues)
  ...
  "analysis results serialization " should "also work for regex with broken ==" in {
    ...
    val result = StatisticsResult(resultKeyOne, ComputedStatistics(Map(analyzer -> metric)))
  ...
  "analysis results serialization with mixed Values" should "fail" in {
    ...
    val analyzerContextWithMixedValues = ComputedStatistics(
      Map(
        Size() -> DoubleMetric(Entity.Column, "Size", "*", Success(5.0)),
        Completeness("ColumnA") ->
    ...
    val analysisResultOne = StatisticsResult(resultKeyOne, analyzerContextWithMixedValues)

    val analysisResultTwo = StatisticsResult(resultKeyTwo, analyzerContextWithMixedValues)
  ...
  "serialization of ApproxQuantile" should "correctly restore it" in {
    ...
    val analyzer = ApproxQuantile("col", 0.5)
    val metric = DoubleMetric(Entity.Column, "ApproxQuantile", "col", Success(0.5))
    val context = ComputedStatistics(Map(analyzer -> metric))
    val result = StatisticsResult(ResultKey(0), context)
  ...
  "serialization of ApproxQuantiles" should "correctly restore it" in {
    ...
    val analyzer = ApproxQuantiles("col", Seq(0.25, 0.5, 0.75))
  ...
  def assertCorrectlyConvertsAnalysisResults(
      analysisResults: Seq[StatisticsResult],
      shouldFail: Boolean = false)
    : Unit = {

    if (shouldFail) {
      intercept[IllegalArgumentException](serialize(analysisResults))
    } else {
...
//        .toEpochSecond(ZoneOffset.UTC)
//
//      val sucessMetricsResultJson = StatisticsResult
